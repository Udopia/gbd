...
# create a new gbd_api object which operates on a database. The file for this database is parameterized in the
...
def __init__(self, database=None):
    if database is not None:
        ...
        raise ValueError("Tried to create api_object without a path to database")
...
def get_group_values(self, name):
    if name is not None:
        return query_search(self.database, '{} like %%%%'.format(name))
    else:
        raise ValueError('No group given')

...
class FeatureExtractor:
    ...
    def __init__(self, api: GBD, features: list, extractor, target_db: str):
        self.api = api
    ...
    def extract(self, instances: pd.DataFrame):
        if self.api.jobs == 1:
            self.extract_sequential(instances)
        else:
            self.extract_parallel(instances)


    def extract_sequential(self, instances: pd.DataFrame):
        for idx, row in instances.iterrows():
            result = self.extractor(row['hash'], row['local'], self.api.get_limits())
            self.save_features(result)


    def extract_parallel(self, instances: pd.DataFrame):
